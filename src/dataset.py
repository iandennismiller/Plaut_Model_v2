...
=== SUMMARY === 
Description     : Defines class for custom dataset for the Plaut Model, and also functions to generate grapheme and phoneme vectors
...
class Plaut_Dataset(Dataset):
    def __init__(self, filepath):
        """
        Initalizes a dataset, calculates log_freq column if required, 
        ...
        assert set(['orth', 'phon', 'type']).issubset(set(self.df.columns)), "ERROR: Dataset file does not contain the minimum required columns"
    ...
    def __getitem__(self, index):
        ...
        return {"type": self.df.loc[index, "type"], # type of word
                ...
                "log_freq": torch.tensor(self.df.loc[index, "log_freq"], dtype=torch.float), # the frequency AFTER log transform
